...
from app.preprocessing.image_utils import crop_rel
from app.rows.config import FIELD_NCELLS, HEADER_ROIS, TABLE_ROIS
from app.rows.header import crop_to_grid_only
from app.rows.grid import detect_rows_by_grid
from app.rows.line_clean import remove_grid_lines
from app.rows.cells import split_cells, _save_cells_list
from app.rows.debug_utils import _save_debug_img


def extract_cells_to_result(
    aligned_image: np.ndarray,
    debug: bool = False,
) -> dict[str, list | list[list]]:
    """
    Вырезает заголовки и строки из выравненного изображения, нарезает клетки,
    возвращает все клетки в памяти. При debug=False ничего не пишет на диск.
    Возвращает: variant (4 cells), date (8), reg_number (8),
    answers (10 rows × 9 cells), repl (10 rows × 9 cells).
    """
    img = aligned_image
    header_debug_base: Path | None = None

    variant_cells: list[np.ndarray] = []
    date_cells: list[np.ndarray] = []
    reg_number_cells: list[np.ndarray] = []

    for name, (x1, y1, x2, y2) in HEADER_ROIS.items():
        crop = crop_rel(img, x1, y1, x2, y2)
        header_key = Path(name).stem
        crop_cells_img = crop_to_grid_only(crop, debug_dir=header_debug_base)
        n = FIELD_NCELLS[header_key]
        cells = split_cells(crop_cells_img, n_cells=n, debug_dir=None)
        if header_key == "variant":
            variant_cells = cells
        elif header_key == "date":
            date_cells = cells
        else:
            reg_number_cells = cells

    left_rows = detect_rows_by_grid(img, TABLE_ROIS["answers"], debug_dir=None)
    right_rows = detect_rows_by_grid(img, TABLE_ROIS["repl"], debug_dir=None)
    n_row_cells = FIELD_NCELLS["answers"]

    answers_cells: list[list[np.ndarray]] = []
    for bbox in left_rows:
        x, y, w, h = bbox
        row_img = img[y : y + h, x : x + w].copy()
        row_img = remove_grid_lines(
            row_img, mode="h", min_len_ratio=0.75, max_thickness=3, close_gaps=0
        )
        cells = split_cells(row_img, n_cells=n_row_cells, debug_dir=None)
        answers_cells.append(cells)

    repl_cells: list[list[np.ndarray]] = []
    for bbox in right_rows:
        x, y, w, h = bbox
        row_img = img[y : y + h, x : x + w].copy()
        row_img = remove_grid_lines(
            row_img, mode="h", min_len_ratio=0.75, max_thickness=3, close_gaps=0
        ...
        cells = split_cells(row_img, n_cells=n_row_cells, debug_dir=None)
        repl_cells.append(cells)

    return {
        "variant": variant_cells,
        "date": date_cells,
        "reg_number": reg_number_cells,
        "answers": answers_cells,
        "repl": repl_cells,
    }
